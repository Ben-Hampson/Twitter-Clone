...
class TweetListView(ListView):  # Homepage
    ...
    def get(self, request, *args, **kwargs):
        ...
        if follows == 0:
            messages.add_message(
                ...
                messages.INFO,
                mark_safe(
                    f"""Find some people to follow: <strong><a href="{explore_url}">Explore</a></strong>!"""
                ),
            )
        return super().get(request, *args, **kwargs)
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated and self.request.path_info != reverse(
            "all-tweets"
        ):
...
class UserTweetListView(ListView):  # Public Profile
    ...
    def get_context_data(self, **kwargs):
        ...
        context["author_followers_count"] = User.objects.filter(
...
class HashtagListView(ListView):
    ...
    def get_queryset(self):
        return get_list_or_404(
            Tweet, message__icontains=f"#{self.kwargs.get('hashtag')}"
        )
