...
def register(request):
    ...
    if request.method == "POST":
        form = UserRegisterForm(
            request.POST
        )  # Puts the POST data into an instance of the form
        if form.is_valid():
            form.save()
...
def follow_user(request, username):
    ...
    if followee != request.user:  # Check they're not trying to follow themselves.
        if (
            followee not in request.user.profile.follows.all()
        ):  # Check user is not already following the followee
            request.user.profile.follows.add(
                followee.id
            )  # Note that it adds the author User id, not profile id
...
def unfollow_user(request, username):
    ...
    if followee != request.user:  # Check they're not trying to follow themselves.
        if (
            followee in request.user.profile.follows.all()
        ):  # Check user is not already following the followee
            ...
            messages.warning(
                request, f"You already do not follow @{followee.username}!"
            )
    else:
